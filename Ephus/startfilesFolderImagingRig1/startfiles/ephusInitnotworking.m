...
xsgStartDirectory = 'G:\DATA\';                 % Default root directory used by XSG program for saving data files. NOTE: This value can be overridden by a saved XSG configuration, which is often desirable.
...
amp(1).amplifierType = 'axopatch_200B';              % One of {'multi_clamp' 'axopatch_200B'}. Identifies type of amplifier device.
...
amp(1).scaledOutputChannelID = [4];      % Number identifying DAQmx channel on board identified by 'scaledOutputBoardID' on which 'scaled output' signal from amplifier channelis input to Ephus
...
amp(1).gainBoardID = [3];                %(AXOPATCH 200B ONLY) Number identifying DAQmx board (e.g. 1 for 'Dev1') on which 'gain' telegraph signal from amplifier channel is input to Ephus
amp(1).gainChannelID = [1];              %(AXOPATCH 200B ONLY) Number identifying DAQmx channel on board identified by 'gainBoardID' on which 'gain' telegraph signal from amplifier channel is input to Ephus
amp(1).modeBoardID = [3];                %(AXOPATCH 200B ONLY) Number identifying DAQmx board (e.g. 1 for 'Dev1') on which 'mode' telegraph signal from amplifier channel is input to Ephus
amp(1).modeChannelID = [2];              %(AXOPATCH 200B ONLY) Number identifying DAQmx channel on board identified by 'modeBoardID' on which 'mode' telegraph signal from amplifier channel is input to Ephus
amp(1).vHoldBoardID = [3];               %(AXOPATCH 200B ONLY) Number identifying DAQmx board (e.g. 1 for 'Dev1') on which '10Vm' telegraph signal from amplifier channel is input to Ephus
amp(1).vHoldChannelID = [3];             %(AXOPATCH 200B ONLY) Number identifying DAQmx channel on board identified by 'vHoldBoardID' on which '10Vm' telegraph signal from amplifier channel is input to Ephus
...
acqChannelNames = {'Trial_number'};                   % Cell array of descriptive names for analog input acquisition channels to configure, e.g. {'Lick Sensor' 'Nose Poke Sensor'}, 
...
acqChannelIDs =  [1];                    % Array of numbers, of length equal to 'acqChannelNames', identifying DAQmx channel number (e.g. 1 for AI1) for each of the named acquisition channels (e.g. [0 1 2 0 1] indicating AI0-2 for first 3 channels  and AI0-1 for last 2 channels, for case of multiple boards). 
...
triggerDestinations = {'PFI1'};               %(REQUIRED) Cell array of one or more DAQmx PFI terminal names configured as the choice of PFI terminals on which Ephus must receive a trigger input signal, on /all/ of the DAQmx boards configured for use by Ephus.
