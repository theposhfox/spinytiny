...
%%% Initialize: get values pertinent to the upcoming calculations 
...
pathname = gui_CaImageViewer.save_directory;
...
elseif strcmpi(load_type, 'Full')
    if ~isempty(strfind(fname, 'SC'))
        ...
        a = regexp(fname, ['\w+00[0]*\d+_\d+'], 'match');
        parsefile = a{1}(1:end-4);
    %     b = regexp(pathname, ['summed'], 'split');
    %     pname = b{1};
        fname = [pname,parsefile];
        CaImage_File_info = imfinfo([fname,'_001_corrected.tif']);
    end
...
ysize = size(gui_CaImageViewer.GCaMP_Image{1},2);
...
%         for j = 1:ysize                                      %%%%% This loop will build a data set representing an image for only the pixels that correspond to the pixels inside the ROI
%             for k = 1:xsize
%                 if ROIreg{i}(j,k) == 0;
%                     Filtered_Data_Image{i}(j,k) = 0;
%                 else 
%                     Filtered_Data_Image{i}(j,k) = gui_CaImageViewer.GCaMP_Image{Frame_num}(j,k);
%                 end
%             end
%         end
...
%                     imnum = ['0', num2str(j)];
%                 end
%                 if j == timecourse_image_number
%                     CaImage_File_info = imfinfo([fname,'_', imnum, '_corrected.tif'])
%                 else
%                 end
%                 for k = 1:length(CaImage_File_info)
%                     wholeimage(:,:,totalimages) = imread([fname, '_', imnum, '_corrected.tif'],k);
...
%                     end
                ...
                for k = 1:length(CaImage_File_info)
%                     try
%                         current_image = imread([fname, '_', imnum, '_corrected.tif'],k);
%                     catch
%                         ['Cannot load ', fname, '_', imnum, '_corrected.tif']
%                     end
...
% % Interval = 70/gui_CaImageViewer.imageserieslength;
% % 
% % Time = 0+Interval:Interval:70;
% % Time = Time-4;
% Time = Time(1:gui_CaImageViewer.imageserieslength);
...
% for i = 1:DendNum
%     counter = 1;
%     for j = DendSpines{i}(1):DendSpines{i}(end)
%         alpha{i}(1:2,counter) = robustfit(deltaDend(i,:),dF_over_F{j});
%         counter = counter + 1;
%     end
% end
% 
...
%     counter = 1;
%     for j = DendSpines{i}(1):DendSpines{i}(end)
%         deltaF_subtracted{j} = dF_over_F{j}-(alpha{i}(1,counter)+alpha{i}(2,counter)*deltaDend(i,:));
%         counter = counter + 1;
%     end
% end
% 
% for i = 1:length(deltaF_subtracted)
%     med = median(deltaF_subtracted{i});
%     spread = std(deltaF_subtracted{i});
%     bin = zeros(1,length(deltaF_subtracted{i}));
%     if ~isempty(deltaF_subtracted{i})
%         [peaks loc] = findpeaks(deltaF_subtracted{i}, 'MinPeakHeight', med+2*spread); 
%         bin(loc) = 1;
%     else
%         peaks = NaN;
%         loc = NaN;
...
%         for i = 1:length(gui_CaImageViewer.PolyROI)
%             if ishandle(gui_CaImageViewer.PolyROI{i})
%                 Poly_deltaF{i} = Poly_Fluorescence_Measurement{i}./nanmean(Poly_Fluorescence_Measurement{i}(baselineFrames));
%                 if twochannels == 1
%                     Poly_dF_over_F{i} = Poly_deltaF{i}./Poly_Red_Measurement{i};
%                 else
%                     Poly_dF_over_F{i} = Poly_deltaF{i};
%                 end
%             end
%         end
%     end
...
try
    ...
    save_fname = [fname, '_Analyzed_By', user];
    evalc([save_fname, '= a']);
    start_dir = cd;
