...
%             flex_plot(1:14, NumCausClustRewSpines, '--', stattype, purple, 2);
    ...
    subplot(3,2,4)
        ...
        b = flex_plot(1:14, MeanCorrelationBetweenAlloDendriticMovementSpines, stattype, gray, 2);
...
%     ylim([0 1.2])
    ...
    DistanceDataChoice = AllDistancesBetweenSuccessSpines;
    CorrelationDataChoice = CorrelationBetweenSuccessSpinesMovePeriods;

    figure('Position', scrsz)
    currentplot = 1;
    subplot(2,4,1)
        xdata = cell2mat(DistanceDataChoice(earlysessions))';
        ydata = cell2mat(CorrelationDataChoice(earlysessions))';
        ydata(isnan(ydata)) = 0;
        %%% K-means clustering 
...
%     %         flex_plot(-1/decay.b,0.368*decay.a, 'ok', 'MarkerFaceColor', 'k') %%% 0.368 corresponds to the decay constant, tau
    ...
    subplot(2,4,currentplot)
        xdata = cell2mat(AllDistancesBetweenAlloDendriticMovementSpines(earlysessions))';
        ydata = cell2mat(CorrelationBetweenAllodendriticMovementSpines(earlysessions))';
        ydata(isnan(ydata)) = 0;
        try
            plot(xdata(ydata>=0.5), ydata(ydata>=0.5), '.', 'Color', lgreen); hold on;
        catch
...
%     %         flex_plot(-1/decay.b,0.368*decay.a, 'ok', 'MarkerFaceColor', 'k') %%% 0.368 corresponds to the decay constant, tau
    ...
    currentplot = 3;
    subplot(2,4,currentplot)
        xdata = cell2mat(DistanceDataChoice(latesessions))';
        ydata = cell2mat(CorrelationDataChoice(latesessions))';
        ydata(isnan(ydata)) = 0;
...
%             legend off
    ...
    currentplot = 4;
    subplot(2,4,currentplot)
        xdata = cell2mat(AllDistancesBetweenAlloDendriticMovementSpines(latesessions))';
        ydata = cell2mat(CorrelationBetweenAllodendriticMovementSpines(latesessions))';
        ydata(isnan(ydata)) = 0;
        try
            plot(xdata(ydata>=0.5), ydata(ydata>=0.5), '.', 'Color', lgreen); hold on;
        catch
...
%             legend off
    ...
    currentplot = 6;
    subplot(2,4,currentplot)
        xdata = cell2mat(AllDistancesBetweenAlloDendriticSpines(earlysessions)');
        ydata = cell2mat(CorrelationBetweenAlloDendriticSpines(earlysessions)');
        ydata(isnan(ydata)) = 0;
        try
            plot(xdata(ydata>=0.5), ydata(ydata>=0.5), '.', 'Color', dred); hold on;
        catch
...
%             plot(-1/decay.b,0.368*decay.a, 'ok', 'MarkerFaceColor', 'k') %%% 0.368 corresponds to the decay constant, tau
    ...
    currentplot = 8;
    subplot(2,4,currentplot)
        xdata = cell2mat(AllDistancesBetweenAlloDendriticSpines(latesessions)');
        ydata = cell2mat(CorrelationBetweenAlloDendriticSpines(latesessions)');
        ydata(isnan(ydata)) = 0;
        try
            plot(xdata(ydata>=0.5), ydata(ydata>=0.5), '.', 'Color', dred); hold on;
        catch
...
%             legend off
    ...
    corrtouse{3} = cell2mat(CorrelationDataChoice(earlysessions));
    disttouse{3} = cell2mat(DistanceDataChoice(earlysessions));
    %         farcorrtouse = CorrelationBetweenFarSpines{sessiontouse};

    corrtouse{4} = cell2mat(CorrelationDataChoice(latesessions));
    disttouse{4} = cell2mat(DistanceDataChoice(latesessions));
